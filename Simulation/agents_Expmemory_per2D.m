...
function [fvx,fvy]=agents_Expmemory_per2D(X,Y,Vx,Vy,N,v0,fluc,wall,len,delt)
parameters
...
f_px=(v0(1:N)-Vx)/gam1;
f_py=(v0(N+1:2*N)-Vy)/gam1;

% Random force
f_rand=fluc*randn(2*N,1);

% The rate of change of velocity
fvx=f_px+f_aax+f_rand(1:N,1)/sqrt(delt);
fvy=f_py+f_aay+f_rand(N+1:2*N,1)/sqrt(delt);
end